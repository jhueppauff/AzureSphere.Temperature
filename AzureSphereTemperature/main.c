...
// include the Grove and Grove Temp Humidity Headers from the MT3620_Grove_Shield_Libary
#include "Grove.h"
...
static int InitPeripheralsAndHandlers(void)
{
	...
	// Open Sensor as input
	Log_Debug("Opening Sensor as input\n");
	// Init 
	int i2cFeed;
	GroveShield_Initialize(&i2cFeed, 230400);
	tempHumiditySensor = GroveTempHumiSHT31_Open(i2cFeed);

	if (tempHumiditySensor < 0) {
		Log_Debug("ERROR: Could not open Sensor: %s (%d).\n", strerror(errno), errno);
		return -1;
	}
